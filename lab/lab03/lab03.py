...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index, res, direction):
        if n == index:
            return res
        if index % 8 == 0 or num_eights(index) > 0:
            return helper(index + 1, res - direction, direction * -1)
        else:
            return helper(index + 1, res + direction, direction)
    return helper(1, 1, 1)
