...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(city_a) if math.sqrt((get_lat(city_a) - lat)**2 + (get_lon(city_a) - lon)**2) < math.sqrt((get_lat(city_b) - lat)**2 + (get_lon(city_b) - lon)**2) else get_name(city_b)
...
def add_chars(w1, w2):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(w1Index, w2Index):
        if w2Index > len(w2)-1:
            return ""
        if w1Index > len(w1)-1:
            return w2[w2Index:]
        if w1[w1Index] != w2[w2Index]:
            return w2[w2Index] + helper(w1Index, w2Index+1)
        else:
            return helper(w1Index+1, w2Index+1)
    return helper(0, 0)
