...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            return insert_into_all(a, b) + subseq_helper(s[1:], -1)
...
def trade(first, second):
    ...
    equal_prefix = lambda: a == b
    while a != b and m < len(first) and n < len(second):
        if a < b:
            a += first[m]
            m += 1
        else:
